-- Adds customer-editable pickup details and staff-managed order notification
-- recipients without resetting existing Grainbuds data.

create table if not exists public.grainbuds_settings (
  key text primary key,
  value jsonb not null default 'null'::jsonb,
  updated_at timestamptz not null default now()
);

alter table public.grainbuds_settings enable row level security;

drop policy if exists "staff read grainbuds_settings" on public.grainbuds_settings;
create policy "staff read grainbuds_settings" on public.grainbuds_settings
  for select to authenticated using (public.grainbuds_is_staff());

drop policy if exists "staff write grainbuds_settings" on public.grainbuds_settings;
create policy "staff write grainbuds_settings" on public.grainbuds_settings
