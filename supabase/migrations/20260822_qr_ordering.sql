-- Track QR-originated orders and attach dine-in orders to a table.
...
alter table public.grainbuds_orders
  drop constraint if exists grainbuds_orders_table_number_check,
  drop constraint if exists grainbuds_orders_order_source_check;

alter table public.grainbuds_orders
  add constraint grainbuds_orders_table_number_check
    check (table_number between 1 and 999),
  add constraint grainbuds_orders_order_source_check
...
-- Keep QR metadata in the row returned after a customer edits an order so
-- follow-up notifications still identify the table correctly.
