-- Require a verified account that owns the order before customer details can
-- be changed. Guest orders are linked to the verified email by the server at
-- login before this function is called.
...
revoke all on function public.grainbuds_update_order_details(uuid, text, text, text, text, text)
  from public, anon, authenticated;
grant execute on function public.grainbuds_update_order_details(uuid, text, text, text, text, text)
  to authenticated;
