...
-- Queue position for one exact, unguessable order id. Returns aggregate
-- preparation data only and never exposes another order or customer.
...
revoke all on function public.grainbuds_order_queue_status(uuid) from public;
grant execute on function public.grainbuds_order_queue_status(uuid) to anon, authenticated;
